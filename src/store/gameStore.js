...
export const useGameStore = create((set, get) => ({
  ...
  sell: (stockId, quantity) => {
    const state = get()
    // 마감 틱과 React 화면 전환 사이에는 수 ms 동안 매도 버튼이 화면에 남을 수 있다.
    // 그 stale 클릭은 종가로 체결하고 이미 만든 일일 보고서까지 다시 계산한다.
    if (state.phase !== 'day' && state.phase !== 'dayReport') return
    const asset = dayData(state)?.stocks.find((stock) => stock.id === stockId)
    ...
    let dailySummaries = state.dailySummaries
    if (state.phase === 'dayReport') {
      const netWorth = calculateNetWorth({ ...state, cash, holdings })
      dailySummaries = state.dailySummaries.map((summary) => summary.cycle === state.cycle && summary.day === state.day
        ? { ...summary, netWorth, cash, change: netWorth - state.dayStartNetWorth }
        : summary)
    }
    set({ cash, holdings, dailySummaries, feedback: { amount: proceeds, profit, id: Date.now(), kind: 'realized' } })
