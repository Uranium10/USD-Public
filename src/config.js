...
// 부채 시스템(B 구조) — 최소 상환액의 계단만으로 주차별 압박을 만든다.
// 총 부채에는 이자를 붙이지 않아, 플레이어가 낸 상환액만큼 숫자가 항상 확실히 줄어든다.
// 계약 수수료를 처음부터 포함한 최종 상환 총액. 진행 중 별도 이자나 숨은 수수료는 붙지 않는다.
export const INITIAL_DEBT = 233800
// 상승 하한(최소 상환액의 바닥값)을 주기별로 직접 지정한 계단형 곡선.
// 초반은 완만하고 후반으로 갈수록 주차 간 증가폭이 커지도록 실측 조정한 값이다.
// 최소 상환만 반복하면 6주차에 정확히 전액 상환된다. 선상환액은 남은 원금을 직접 줄인다.
export const FLOOR_BY_CYCLE = [18800, 23000, 29000, 39000, 53000, 71000]
