...
import { COIN_ASSET_ID, DAYS_PER_CYCLE, EPILOGUE_CYCLE, SISYPHUS_STOCK_ID } from './config.js'
import { stageEngine } from './engine/StageEngine.js'
import { getMinPayment } from './logic/debtSystem.js'
import { selectBriefingRumors } from './logic/informationBriefing.js'
...
function Premarket() {
  ...
  const completePurchase = () => {
    ...
    if (selectedRumors.length) {
      ...
      if (!purchased) return
      purchasedIds = purchased.map((rumor) => rumor.id)
      playCashOut()
      setFlashingRumorIds(purchasedIds)
      setSelectedRumorIds([])
      window.setTimeout(() => setFlashingRumorIds([]), 450)
    }
    state.showOverlay({ type: 'dayBriefing', title: `${state.day}일차 정보 브리핑`, rumorIds: purchasedIds })
  }
...
function Settlement() {
  ...
  const debtAfterPayment = Math.max(0, state.debt - payAmount)
  ...
  return <section className="modal-card settlement-card">
    ...
    {canPayMin ? <>
      ...
      <p className="settlement-preview">이번에 {money(payAmount)} 상환 시 총부채 {money(debtAfterPayment)} <span className="green">(−{money(payAmount)})</span></p>
...
function Overlay() {
  ...
  const visibleRumors = selectBriefingRumors(state.purchasedRumors, state.overlay.rumorIds)
  return <div className="overlay-backdrop"><div className="overlay-modal">{state.overlay.title && <h2>{state.overlay.title}</h2>}{state.overlay.text && <p>{state.overlay.text}</p>}{information && <div className="information-list">{visibleRumors.length === 0 && <p className="muted">{state.overlay.type === 'dayBriefing' ? '오늘 구입한 정보가 없습니다.' : '구입한 정보가 없습니다.'}</p>}{visibleRumors.map((rumor) => <article key={rumor.id}><small>{rumor.source} · 신뢰도 {Math.round(rumor.accuracy * 100)}%</small><p>{rumor.text}</p></article>)}</div>}{state.overlay.type === 'dayBriefing' ? <button className="primary" onClick={state.startDay}>{state.day}일차 시작</button> : <button className="primary" onClick={state.closeOverlay}>닫기</button>}</div></div>
}
