...
export default function MarketDesktop() {
  ...
  const realizedFeedback = state.feedback?.kind === 'realized'
      ...
      {activeApp === 'notepad' ? <InformationNotepad rumors={state.purchasedRumors} />
        ...
        <section className="account-strip">
          <div className="asset-cell">
            ...
            {state.feedback && !realizedFeedback && <div key={state.feedback.id} className={`asset-feedback ${state.feedback.amount >= 0 ? 'gain' : 'loss'}`}>{state.feedback.amount >= 0 ? '+' : ''}{money(state.feedback.amount)}</div>}
          </div>
          <div className="asset-cell"><small>현금</small><strong>{money(state.cash)}</strong>{realizedFeedback && <div key={state.feedback.id} className="asset-feedback cash-feedback gain">현금 +{money(state.feedback.amount)}</div>}</div>
