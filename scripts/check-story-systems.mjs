...
import { selectBriefingRumors } from '../src/logic/informationBriefing.js'
...
assert(selectBriefingRumors(briefingRumors).length === 2 && selectBriefingRumors(briefingRumors, []).length === 0, '전체 정보 보기와 정보 없이 진행의 표시 범위가 잘못됐습니다.')
...
const closingMarket = generateMarketCycle({ cycle: 1, seed: 811 })
const closingStock = closingMarket.days[0].stocks.find((stock) => stock.assetType !== 'coin')
const closingPrice = closingStock.startPrice
const closingNetWorth = 1000 + closingPrice * 10
useGameStore.setState({
  market: closingMarket, phase: 'dayReport', cycle: 1, day: 1, cash: 1000,
  holdings: { [closingStock.id]: { quantity: 10, average: closingPrice } },
  currentPrices: { [closingStock.id]: closingPrice }, dayStartNetWorth: closingNetWorth,
  dailySummaries: [{ cycle: 1, day: 1, cash: 1000, netWorth: closingNetWorth, change: 0 }],
})
const closingSale = useGameStore.getState().sell(closingStock.id, 10)
const closingState = useGameStore.getState()
assert(closingSale?.proceeds === closingPrice * 10 && closingState.cash === 1000 + closingSale.proceeds, '마감 화면 전환 직전 매도가 종가로 현금화되지 않습니다.')
assert(!closingState.holdings[closingStock.id] && closingState.dailySummaries.at(-1).cash === closingState.cash, '마감 직전 매도 후 보고서가 새 현금·보유량으로 갱신되지 않습니다.')
assert(closingState.feedback.amount === closingSale.proceeds && closingState.feedback.kind === 'realized', '매도 피드백이 현금 유입액을 표시하지 않습니다.')
